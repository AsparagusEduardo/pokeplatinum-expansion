...
static void BoxPokemon_SetDefaultMoves(BoxPokemon *boxMon)
{
    ...
    SpeciesLearnset *monLevelUpMoves = Heap_Alloc(HEAP_ID_SYSTEM, sizeof(SpeciesLearnset));
    ...
    for (int i = 0; monLevelUpMoves->entries[i].move != LEARNSET_SENTINEL_ENTRY; i++) {
        if ((monLevelUpMoves->entries[i].level) <= monLevel) {
            u16 monLevelUpMoveID = monLevelUpMoves->entries[i].move;
...
u16 Pokemon_LevelUpMove(Pokemon *mon, int *index, u16 *moveID)
{
    ...
    if (monLevelUpMoves->entries[*index].move == LEARNSET_SENTINEL_ENTRY) {
        Heap_Free(monLevelUpMoves);
        return MOVE_NONE;
    }

    while ((monLevelUpMoves->entries[*index].level) != monLevel) {
        (*index)++;
        if (monLevelUpMoves->entries[*index].move == LEARNSET_SENTINEL_ENTRY) {
            Heap_Free(monLevelUpMoves);
            return MOVE_NONE;
    ...
    if ((monLevelUpMoves->entries[*index].level) == monLevel) {
        *moveID = monLevelUpMoves->entries[*index].move;
        (*index)++;
...
int Pokemon_LoadLevelUpMoveIdsOf(int monSpecies, int monForm, u16 *monLevelUpMoveIDs)
{
    ...
    while (monLevelUpMoves->entries[result].move != LEARNSET_ALL_SLOTS_FILLED) {
        monLevelUpMoveIDs[result] = monLevelUpMoves->entries[result].move;
        result++;
    }
...
void Pokemon_LoadLevelUpMovesOf(int monSpecies, int monForm, SpeciesLearnset *monLevelUpMoves)
{
    monSpecies = Pokemon_GetFormNarcIndex(monSpecies, monForm);
